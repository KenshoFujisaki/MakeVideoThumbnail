...
for file in `eval "$input_files_cmd"`; do
  ...
  if [ ${#hms[*]} -eq 0 ]; then
    printf "error (duration is not found).\n" | tee -a $log_file_path
    continue
  fi
  duration_sec=$((${hms[0]} * 3600 + ${hms[1]} * 60 + ${hms[2]}))
  ...
  thumb_fps=$(($frames / $tiles)) #3
  if [ $thumb_fps -gt 1000 ]; then #2
    thumb_fps=1000
  fi
  $ffmpeg -y -i "$file" -vf thumbnail=${thumb_fps},tile=${tiles_h}x${tiles_v},scale=1920:-1 "${file%.*}_thumb.jpg" >/dev/null 2>&1 
